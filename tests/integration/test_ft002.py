...
#####
"""
    logger.debug("init_board start")
    logger.debug(BoardConfig.LENGTH)
...
@pytest.mark.asyncio
async def test_ft002_move_scenario():
    """
...
@patch("server.initialize_board", new=opened_board_map)
@pytest.mark.asyncio
async def test_ft002_business_rule_opened_tile_only():
    """
    비즈니스 규칙 검증:
...
@patch("server.initialize_board", new=opened_board_map)
@pytest.mark.asyncio
async def test_ft002_business_rule_dead_cursor():
    """
    비즈니스 규칙 검증:
    - 죽은 cursor는 이동할 수 없다.
    """
...
@patch("server.initialize_board", new=opened_board_map)
@pytest.mark.asyncio
async def test_ft002_state_change_position_and_score():
    """
    상태 변화 검증:
    ...
    with PytestTCM(app).append_client(CL_A) as tcm:
        ...
        assert pos_before != pos_after, f"{cursor_before}, {cursor_after}"
